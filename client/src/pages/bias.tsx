      ...
      {selectedMembros.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {selectedMembros.map((m) => {
            ...
            return (
            <Badge key={m.id} variant="secondary" className="gap-1 bg-blue-600 pr-1 text-white hover:bg-blue-600">
              {getMembroNome(m)}
              {isPending && (
              ...
              <button type="button" onClick={() => { void toggleMembro(m.id); }} className="rounded-full text-white hover:bg-white/20">
                <X className="w-3 h-3" />
              </button>
            </Badge>
            );
