...
Responda sempre em português brasileiro, de forma clara e objetiva.`;
  ...
  app.get("/api/proposals", async (req, res) => {
    try {
      const items = await storage.getAllProposals();
      res.json(items);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.post("/api/proposals", async (req, res) => {
    try {
      const item = await storage.createProposal(req.body);
      res.json(item);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.get("/api/proposals/:id", async (req, res) => {
    try {
      const item = await storage.getProposal(req.params.id);
  ...
  app.patch("/api/proposals/:id", async (req, res) => {
    try {
      const item = await storage.updateProposal(req.params.id, req.body);
      if (!item) return res.status(404).json({ error: "Proposta não encontrada" });
      res.json(item);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
  ...
  app.delete("/api/proposals/:id", async (req, res) => {
    try {
      const ok = await storage.deleteProposal(req.params.id);
  ...
  app.post("/api/proposals/generate", async (req, res) => {
    try {
      const { opa_id, bia_id, instrucoes } = req.body;
      if (!opa_id) return res.status(400).json({ error: "opa_id é obrigatório" });

      const [opaData, biasData] = await Promise.all([
        directusFetch("tipos_oportunidades", `filter[id][_eq]=${opa_id}`),
        bia_id ? directusFetch("bias_projetos", `filter[id][_eq]=${bia_id}`) : Promise.resolve([]),
      ]);
      const opa = opaData[0] || {};
      const bia = biasData[0] || {};

      const contextPrompt = `
OPA (Oportunidade de Aliança):
- Nome: ${opa.nome_oportunidade || "N/A"}
- Tipo: ${opa.tipo || "N/A"}
- País: ${opa.pais || "Brasil"}
- Valor: R$ ${Number(opa.valor_origem_opa || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}
- Objetivo da Aliança: ${opa.objetivo_alianca || "N/A"}
- Núcleo de Aliança: ${opa.nucleo_alianca || "N/A"}
- Perfil do Aliado: ${opa.perfil_aliado || "N/A"}
- Descrição: ${opa.descricao || "N/A"}
${bia.nome_bia ? `
BIA (Projeto de Aliança) Vinculado:
- Nome: ${bia.nome_bia}
- Objetivo: ${bia.objetivo_alianca || "N/A"}
- Localização: ${bia.localizacao || "N/A"}` : ""}
${instrucoes ? `\nInstruções adicionais do usuário: ${instrucoes}` : ""}`;

      const systemPrompt = `Você é um especialista em propostas comerciais da Built Alliances, uma rede de alianças estratégicas no mercado de construção e engenharia. Gere uma proposta profissional em HTML completo (documento completo com DOCTYPE, head, body) com o seguinte estilo visual:
- Cores: fundo #001D34 (navy), dourado #D7BB7D, texto branco
- Design futurista, elegante e profissional
- Seções: Cabeçalho com logo "Built Alliances" e data, Apresentação, Detalhes da Oportunidade, Proposta de Valor, Próximos Passos, Rodapé
- Use tabelas HTML para dados financeiros quando aplicável
- Inclua um assunto de email sugerido no formato: {"assunto": "..."} em um comentário HTML no início: <!-- ASSUNTO: ... -->
A proposta deve ser persuasiva, profissional e em português brasileiro. Gere apenas o HTML, sem explicações adicionais.`;

      const response = await openai.chat.completions.create({
        model: "gpt-4o",
        messages: [
          { role: "system", content: systemPrompt },
          { role: "user", content: `Gere uma proposta comercial completa para a seguinte oportunidade:\n${contextPrompt}` }
        ],
        max_tokens: 4000,
        temperature: 0.7
      });
      ...
      const subjectMatch = html.match(/<!--\s*ASSUNTO:\s*(.+?)\s*-->/i);
      const assunto = subjectMatch ? subjectMatch[1].trim() : `Proposta de Aliança: ${opa.nome_oportunidade || "Nova OPA"}`;

      res.json({ success: true, html_content: html, assunto_email: assunto, opa, bia });
    } catch (error: any) {
  ...
  app.post("/api/proposals/:id/send-email", async (req, res) => {
    try {
      ...
      const { email_to, nome_to, assunto } = req.body;
      const toEmail = email_to || proposal.email_destinatario;
      const toNome = nome_to || proposal.nome_destinatario || toEmail;
      const subject = assunto || proposal.assunto_email || `Proposta Built Alliances: ${proposal.nome_proposta}`;

      if (!toEmail) return res.status(400).json({ error: "Email destinatário é obrigatório" });
      ...
      if (!smtpHost || !smtpUser || !smtpPass) {
        return res.status(400).json({
          error: "Configuração SMTP incompleta. Configure as variáveis SMTP_HOST, SMTP_USER e SMTP_PASS.",
      ...
      await transporter.sendMail({
        from: `"Built Alliances" <${smtpFrom}>`,
        to: toNome ? `"${toNome}" <${toEmail}>` : toEmail,
        subject,
        html: proposal.html_content || "<p>Proposta Built Alliances</p>",
      });
      ...
      await storage.updateProposal(proposal.id, {
        ...
        nome_destinatario: toNome,
      });

      res.json({ success: true, message: `Email enviado para ${toEmail}` });
    } catch (error: any) {
  ...
  app.get("/api/smtp/status", async (req, res) => {
    res.json({
      ...
      host: process.env.SMTP_HOST ? process.env.SMTP_HOST : null,
