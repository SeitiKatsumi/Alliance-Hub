...
import { LucideIcon, Plus, Pencil, Trash2, FileText, Upload, X, Paperclip, ExternalLink } from "lucide-react";

export type TipoDoc = { label: string };
export type AliancaConfig = { key: string; label: string; tipos: TipoDoc[] };

export interface AliancaDocsPageConfig {
  modulo: string;
  titulo: string;
  subtitulo: string;
  accentColor: string;
...
type Doc = {
  id: string; modulo?: string; bia_id?: string; alianca_tipo?: string;
  tipo_documento?: string; descricao?: string; arquivo_ids?: string[];
  arquivos?: AnexoFile[]; created_at?: string;
};
...
function DocSheet({ config, aliancaTipo, biaId, bias, doc, onClose }: DocSheetProps) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      ...
      if (doc) return apiRequest("PATCH", `/api/alianca-docs/${doc.id}`, payload);
      return apiRequest("POST", "/api/alianca-docs", payload);
    },
    onSuccess: () => {
  ...
  const removeExistingId = (id: string) => setForm(f => ({ ...f, arquivo_ids: f.arquivo_ids.filter(x => x !== id) }));

  const inputCls = "bg-white/5 border-white/10 text-white placeholder:text-white/25 focus:border-white/20 text-sm h-9";
  const labelCls = "text-[11px] text-white/40 uppercase tracking-wider mb-1 block";

  return (
    <Sheet open onOpenChange={o => { if (!o) onClose(); }}>
      <SheetContent side="right" className="w-full sm:max-w-xl flex flex-col p-0" style={{ background: "#020d1a", borderLeft: `1px solid ${accentColor}20` }}>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div>
            ...
            <Textarea
              ...
              className="bg-white/5 border-white/10 text-white placeholder:text-white/25 focus:border-white/20 text-sm resize-none"
              rows={3}
            ...
            <div className="space-y-1.5">
              {existingArquivos.filter(a => form.arquivo_ids.includes(a.id)).map(a => (
                <div key={a.id} className="flex items-center gap-2 rounded-md px-3 py-2" style={{ background: "#ffffff08", border: "1px solid #ffffff10" }}>
                  <Paperclip className="w-3.5 h-3.5 shrink-0" style={{ color: accentColor }} />
                  ...
                  <button onClick={() => removeExistingId(a.id)} className="text-white/30 hover:text-red-400 transition-colors"><X className="w-3.5 h-3.5" /></button>
                  <a href={a.url} target="_blank" rel="noreferrer" className="text-white/30 hover:text-white/60 transition-colors"><ExternalLink className="w-3.5 h-3.5" /></a>
                </div>
              ))}
            </div>
          )}
          ...
          {pendingFiles.length > 0 && (
            <div className="space-y-1.5">
              {pendingFiles.map((f, i) => (
                <div key={i} className="flex items-center gap-2 rounded-md px-3 py-2" style={{ background: `${accentColor}10`, border: `1px solid ${accentColor}20` }}>
                  <Paperclip className="w-3.5 h-3.5 shrink-0" style={{ color: accentColor }} />
                  <span className="text-xs text-white/70 flex-1 truncate">{f.name}</span>
                  <button onClick={() => setPendingFiles(p => p.filter((_, j) => j !== i))} className="text-white/30 hover:text-red-400 transition-colors"><X className="w-3.5 h-3.5" /></button>
                </div>
              ))}
            </div>
          )}
          ...
          <button onClick={() => fileRef.current?.click()} className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg border border-dashed text-sm transition-colors" style={{ borderColor: `${accentColor}30`, color: `${accentColor}80` }} data-testid="btn-upload-arquivo">
            <Upload className="w-3.5 h-3.5" /> Anexar arquivo(s)
          </button>
        </div>

        <div className="px-6 py-4 border-t border-white/5 flex gap-3 shrink-0">
          <Button variant="ghost" onClick={onClose} className="flex-1 text-white/50 hover:text-white border border-white/10">Cancelar</Button>
          <Button
      ...
      <div>
        <div className="flex items-center justify-between mb-5">
          ...
          <Button
            ...
            style={{ background: accentColor, color: "#001D34" }}
            data-testid={`btn-novo-doc-${activeTab}`}
          >
          ...
          {tabDocs.map(doc => (
            ...
            >
              <div className="flex items-start gap-3">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ background: `${accentColor}10`, border: `1px solid ${accentColor}20` }}>
                ...
                <div className="flex gap-1 shrink-0">
                  ...
                  <button onClick={() => setDeletingDoc(doc)} className="w-7 h-7 rounded flex items-center justify-center text-white/30 hover:text-red-400 hover:bg-red-500/10 transition-colors" data-testid={`btn-deletar-doc-${doc.id}`}>
      ...
      <AlertDialog open={!!deletingDoc} onOpenChange={o => { if (!o) setDeletingDoc(undefined); }}>
        <AlertDialogContent style={{ background: "#020d1a", border: "1px solid #ffffff10" }}>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/50">"{deletingDoc?.tipo_documento}" será removido permanentemente.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel className="border-white/10 text-white/70 hover:text-white">Cancelar</AlertDialogCancel>
            <AlertDialogAction onClick={() => deletingDoc && deleteMutation.mutate(deletingDoc.id)} className="bg-red-600 hover:bg-red-700 text-white">Excluir</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
