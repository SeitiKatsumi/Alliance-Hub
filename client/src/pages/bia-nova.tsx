...
const steps=["Dados da BIA","Base econômica inicial","MAP Inicial","Revisão"];
...
export default function BiaNovaPage(){
  ...
  const brand=useBiaBrand(id,form.nome_bia,step===3);
  ...
  return <main className="mx-auto max-w-[1500px] space-y-8 px-4 py-6 sm:px-8">
    ...
    <div className={`grid min-w-0 items-start gap-8 ${step<=1 || form.map_inicial.modeloCalculo===5 && step>=2?"":"xl:grid-cols-[minmax(0,1fr)_320px]"}`}>
      <fieldset disabled={busy || concluding} className="min-w-0 space-y-6">
        ...
        {step===3 && preview.map && preview.teamPending && <p role="status" className="rounded-lg bg-amber-50 p-4 text-sm text-amber-900">O MAP Inicial foi calculado. Para concluir a estruturação, defina Aliado BUILT e Diretor de Aliança na Base econômica inicial. Autor é opcional.</p>}
    ...
    <footer className="sticky bottom-0 flex flex-wrap items-center justify-between gap-3 border-t bg-background py-4"><Button variant="outline" disabled={step===0 || busy} onClick={()=>setStep(s=>s-1)}>Voltar</Button><div className="flex flex-wrap gap-2">{step===3 && <BiaPdfDialog reviewRef={reviewRef} name={form.nome_bia} disabled={busy || !preview.map} modelFive={form.map_inicial.modeloCalculo===5} certified={form.selo_certified_alliance===true} brandUrl={brand.url} brandCode={brand.code}/>}<Button variant="outline" disabled={busy || !form.nome_bia.trim()} onClick={()=>save()}>{busy?"Salvando…":"Salvar rascunho"}</Button>{step<3?<Button disabled={busy || (step===0 && !form.nome_bia.trim()) || (step===1 && !preview.map)} onClick={()=>setStep(s=>s+1)}>{step===1 && form.map_inicial.modeloCalculo===5?"Gerar MAP Inicial":"Continuar"}<ArrowRight className="ml-2 h-4 w-4"/></Button>:<Button disabled={busy || generalPending.length>0 || !preview.map || preview.teamPending || !form.nome_bia.trim() || members.isError || defaults.isError} onClick={()=>save(true)}>Concluir estruturação</Button>}</div></footer>
