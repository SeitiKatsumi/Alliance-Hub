...
- `shared/bia-setup.ts` define o contrato `estrutura_bia` de rascunho, validações, adaptação legada e consolidação monetária. Estrutura Jurídica permite Societária/Contratual, responsável com CPF/CNPJ, empresa/instrumento, sócios formais e conta. Sócio formal externo não cria participação, CPP ou acesso.
