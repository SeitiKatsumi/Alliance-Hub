...
export default function BiaNovaPage(){
  ...
  const preview=useMemo(()=>{try{
    ...
    validateInitialClassifications(map.participantes);return {map,error:"",teamPending:!team.autor_bia || !team.aliado_built || !team.diretor_alianca};
  ...
  return <main className="mx-auto max-w-[1500px] space-y-8 px-4 py-6 sm:px-8">
    ...
    <div className={`grid min-w-0 items-start gap-8 ${step<=1 || form.map_inicial.modeloCalculo===5 && step>=2?"":"xl:grid-cols-[minmax(0,1fr)_320px]"}`}>
      <fieldset disabled={busy || concluding} className="min-w-0 space-y-6">
        ...
        {step===3 && preview.map && preview.teamPending && <p role="status" className="rounded-lg bg-amber-50 p-4 text-sm text-amber-900">O MAP Inicial foi calculado. Para concluir a estruturação, defina Autor, Aliado BUILT e Diretor de Aliança na Base econômica inicial.</p>}
