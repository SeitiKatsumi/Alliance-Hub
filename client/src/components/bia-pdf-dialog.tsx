...
export function BiaPdfDialog({reviewRef,name,disabled,modelFive,certified,brandUrl,brandCode}:{reviewRef:RefObject<HTMLDivElement|null>;name:string;disabled:boolean;modelFive:boolean;certified:boolean;brandUrl:string;brandCode:string|null}) {
  ...
  return <Dialog open={open} onOpenChange={value=>{setOpen(value);setError("");}}>
    ...
    <DialogContent className="w-[calc(100%-2rem)] sm:max-w-lg">
      ...
      <p className="text-xs text-muted-foreground">Rodapé em todas as páginas: marca BIA{certified?" e selo Aliança Certificada":". O selo Aliança Certificada aparece quando marcado nos Dados da BIA"}.</p>
      {!sections.length && <p role="status" className="text-sm text-amber-700">Selecione pelo menos uma seção.</p>}
      {error && <p role="alert" className="text-sm text-destructive">{error}</p>}
      <DialogFooter className="gap-2"><Button type="button" variant="outline" onClick={()=>setOpen(false)}>Cancelar</Button><Button type="button" disabled={disabled || !sections.length || !brandUrl} onClick={()=>{
        if(!reviewRef.current)return;
        if(printBiaSummary(reviewRef.current,name,sections,certified,brandUrl,brandCode)){setOpen(false);setError("");}
