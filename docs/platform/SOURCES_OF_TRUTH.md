...
| Estrutura jurídica e ativos informativos da BIA | PostgreSQL `bia_estrutura_versoes`; antes da conclusão, `bia_estruturacao_rascunhos.dados.estrutura_bia` | revisão/PDF e adaptação somente leitura de `info_comercial` legado | revisão concorrente, histórico imutável; conta exige permissão bancária; nenhum efeito automático no MAP, MOU emitido ou Carteira |
| Seleção de ativo da Carteira para BIA | cadastro original em `inventario_imoveis`; referência `ativos[].carteiraImovelId` e cópia descritiva na estrutura da BIA | formulário, histórico e resumo autorizado | seleção exige propriedade/administração explícita; múltiplos ativos sem duplicar origem; sem sincronizar valores, propriedade, documentos ou `bia_imovel_origens` |
| Governança da BIA | titulares/papéis normalizados Directus; início conhecido/responsabilidades em `bia_estrutura_versoes` | histórico de estrutura e `bia_map_versoes` | chave de autorização interna `diretoria` preservada; sócio formal externo não é participante |
