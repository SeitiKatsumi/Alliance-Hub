...
async function ensureVitrineFields() {
  const fields = [
    ...
    { field: "ramo_atuacao", type: "string", meta: { interface: "input", display: "raw", hidden: false, note: "Ramo de atuação profissional" }, schema: { is_nullable: true } },
  ];
  for (const fieldDef of fields) {
    try {
