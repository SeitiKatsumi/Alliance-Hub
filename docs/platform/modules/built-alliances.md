...
- No detalhe, **Dados da BIA** substitui o atalho jurídico, mantendo acesso `configuracao_bia:view`. Modal somente leitura com Resumo, Marcas e MAP Inicial: identidade/fase/código vêm do detalhe Directus autorizado, valores do snapshot oficial e PDFs de MAP das versões históricas `tipo=zero`. Sem código oficial, os downloads personalizados ficam bloqueados; nunca usar o ID interno. Resumo e marcas usam o layout PDF aprovado, em paisagem; marcas vertical/horizontal também podem ser baixadas em PNG. Não grava, recalcula, certifica ou modifica capa, banco ou documentos anteriores. Jurídico/ativos continuam na página Editar com as permissões existentes. Testes de exportação cobrem imagens decodificadas antes da impressão e versões iniciais separadas do MAP Atual.
...
- Campos jurídicos, comerciais, bancários e do quadro societário exibem exemplos ilustrativos, sem persistir placeholders. Ao abrir o formulário jurídico editável, Nome Fantasia vazio recebe automaticamente o nome atual da BIA em `juridico.info.nome_fantasia`, sem botão. Aguarda o carregamento dos dados salvos; nomes existentes são preservados. O padrão é aplicado uma vez por montagem do formulário, permite editar/limpar e não sincroniza renomeações posteriores. Consulta sem edição não modifica o estado. Persiste somente pelo salvamento/revisão existente e aparece nos resumos/PDF como qualquer Nome Fantasia cadastrado. Sem nova fonte de verdade, migração, permissão, e-mail ou alteração de documentos emitidos.
- O quadro societário exibe o total de fichas de sócios formais (`juridico.socios.length`) e quantas estão sem nome preenchido, atualizado ao incluir/preencher/remover. Não representa participantes, CPPs, Governança, convites ou aceites do MAP; nenhuma contagem nova é gravada no banco.
- Na modalidade Societária, Objeto social vazio recebe uma vez no formulário editável o padrão aprovado pelo usuário: `Administração de bens próprios, incluindo imóveis, veículos, participações societárias e outros ativos patrimoniais, gestão de ativos e investimentos.` O campo usa caixa de texto multilinha editável; preserva objetos existentes e alterações manuais. Não preenche no modelo Contratual ou em consulta, não altera documentos históricos e só persiste pelo salvamento da estrutura/rascunho existente.
